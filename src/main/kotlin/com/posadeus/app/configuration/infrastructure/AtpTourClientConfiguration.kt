...
open class AtpTourClientConfiguration {
  ...
  @Bean
  open fun atpTourGetClient(
    @Value("\${client.atp-tour.uri}") atpTourURI: String,
    atpTourRestTemplate: RestTemplate
  ): AtpTourClient =
    RestAtpTourClient(
      atpTourURI,
      atpTourRestTemplate
  ...
  open fun atpTourRestTemplate(
    ...
    @Value("\${client.atp-tour.http.connection-timeout}") connectionTimeout: Int
  ): RestTemplate {
