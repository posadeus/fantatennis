...
class SqlRetrieveFantaTournamentsRepositoryIT {
  ...
  private lateinit var repository: RetrieveFantaTournamentsRepository
  ...
  fun setUp() {
    ...
    repository = SqlRetrieveFantaTournamentsRepository(cachedFantaTournamentDao)
  ...
  inner class RetrieveBy {
    ...
    fun `no tournament found`() {
      ...
      assertThat(repository.retrieveBy(A_FANTA_TOURNAMENT_ID)).isEqualTo(InvalidFantaTournament)
    }
    ...
    fun `retrieve by id`() {
      ...
      assertThat(repository.retrieveBy(1)).isEqualTo(expected)
    }
  }

  companion object {

    private const val A_FANTA_TOURNAMENT_ID = 999
  }
}
